...
import { insertTeamIfNotExists, listTeams } from "@/models/team";
...
program
	...
	.option(
		"--source <source>",
		"Fonte para buscar as partidas, por enquanto apenas espn"
	)
		...
		if (id === "all") {
			...
			for (const team of teams) {
				for (let season = from; season <= to; season++) {
					await saveSeasonSchedule(
						Number.parseInt(team.id),
						leagues,
						season
					);
				}
...
async function saveSeasonSchedule(
	...
	const matches = await espnMatchUpdater.getTeamSchedule(
		teamId,
		season,
		leagues
	);
	...
	matches.forEach(async (match) => {
		...
		console.log(
			`inserido partida: liga: ${match.league.name} ${match.id}: ${match.homeCompetitor.team.name} ${match.homeCompetitor.score} x ${match.awayCompetitor.score} ${match.awayCompetitor.team.name}`
		);
	});
}
