...
from .match import Match, Event, EventType
...
class MatchService:
    ...
    def process_league_schedule(self, league: str) -> None:
        ...
        for event in day_schedule.events:
            ...
            if match.status_name != "STATUS_SCHEDULED":
                ...
                match_events = []
                for match_event in matchSummary.events:
                    if "gol" in match_event["type"]["text"].lower() or "goal" in match_event["type"]["text"].lower():
                        new_match_event_type = EventType(match_event["type"]["id"], match_event["type"]["text"])
                        new_match_event = Event(new_match_event_type, "", int(match_event["clock"]["value"]), int(match_event["team"]["id"]), match_event["participants"][0]["athlete"]["displayName"])

                        match_events.append(new_match_event)
    ...
    def schedule_match_if_necessary(self, matches: List[Match], league: str) -> None:
        for match in matches:
            match_date = datetime.strptime(match.date, '%Y-%m-%dT%H:%MZ').replace(tzinfo=timezone.utc, microsecond=0, second=0)
        ...
        self.__scheduler.schedule(datetime.now(timezone.utc).replace(microsecond=0, hour=10, minute=0, second=0) + timedelta(days=1, microseconds=0), partial(self.process_league_schedule, league))